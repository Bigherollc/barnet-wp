...
class YamlHelper
{
    ...
    const FALSE = "false";
    const YAMLSTRING = "YAMLString";
    const YAMLSEQ = "YAMLSeq";
    const YAMLMAP = "YAMLMap";
    /**
    ...
    private function dumpNode($key, $value, $indent, $previous_key = -1, $first_key = 0, $source_array = null)
    {
        ...
        if (in_array($value, array('true', 'TRUE', self::FALSE, 'FALSE', 'y', 'Y', 'n', 'N', 'null', 'NULL'), true)) {
            $value = $this->doLiteralBlock($value, $indent);
        }
        if (trim($value) != $value) {
            $value = $this->doLiteralBlock($value, $indent);
    ...
    private function toType($value)
    {
        ...
        if (in_array(strtolower($value), array(self::FALSE, 'off', '-', 'no', 'n'))) {
            return false;
        }
    ...
    private function inlineEscape($inline)
    {
        ...
        if (preg_match_all($regex, $inline, $strings)) {
            ...
            $inline = preg_replace($regex, self::YAMLSTRING, $inline);
        }
        ...
        do {
            ...
            while (preg_match('/\[([^{}\[\]]+)\]/U', $inline, $matchseqs)) {
                $seqs[] = $matchseqs[0];
                $inline = preg_replace('/\[([^{}\[\]]+)\]/U', (self::YAMLSEQ . (count($seqs) - 1) . 's'), $inline, 1);
            }

            // Check for mappings
            while (preg_match('/{([^\[\]{}]+)}/U', $inline, $matchmaps)) {
                $maps[] = $matchmaps[0];
                $inline = preg_replace('/{([^\[\]{}]+)}/U', (self::YAMLMAP . (count($maps) - 1) . 's'), $inline, 1);
            }
        ...
        while (1) {
            ...
            if (!empty($seqs)) {
                foreach ($explode as $key => $value) {
                    if (strpos($value, self::YAMLSEQ) !== false) {
                        foreach ($seqs as $seqk => $seq) {
                            $explode[$key] = str_replace((self::YAMLSEQ . $seqk . 's'), $seq, $value);
                            $value = $explode[$key];
                        }
                    }
                }
            ...
            if (!empty($maps)) {
                foreach ($explode as $key => $value) {
                    if (strpos($value, self::YAMLMAP) !== false) {
                        foreach ($maps as $mapk => $map) {
                            $explode[$key] = str_replace((self::YAMLMAP . $mapk . 's'), $map, $value);
                            $value = $explode[$key];
                        }
                    }
                }
            ...
            if (!empty($saved_strings)) {
                foreach ($explode as $key => $value) {
                    while (strpos($value, self::YAMLSTRING) !== false) {
            ...
            foreach ($explode as $key => $value) {
                ...
                if (strpos($value, self::YAMLSTRING) !== false) {
                    $finished = false;
                    break;
                }
            }

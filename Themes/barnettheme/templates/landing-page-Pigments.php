<?php
/* Template Name: Pigments Landing Page */
global $wp;
...
$queryPostsPigments = new WP_Query($args);
$postsPigmentsDefault = $queryPostsPigments->posts;

$postsPigments = array();
$metaPostManager = new BarnetPostMetaManager($postsPigmentsDefault);
$relationshipManager = new BarnetRelationshipManager();
$relationshipManager->syncTerm();
foreach ($postsPigmentsDefault as $postPigmentsDefault) {
    //Remove product on list
    $product_only_for_code_list = 0;
    $product_only_for_code_list = get_post_meta(intval($postPigmentsDefault->ID), 'product_only_for_code_list', TRUE);
    if (intval($product_only_for_code_list) == 1) {
        continue;
    }
    $productEntity = new ProductEntity(
        $postPigmentsDefault->ID,
        true,
        array(
            'post' => $postPigmentsDefault,
            'meta' => $metaPostManager->getMetaData($postPigmentsDefault->ID)
        )
    );
    $productEntity->setRelationshipManager($relationshipManager);
    if ($productEntity->checkRoleAndRegion()) {
        $postsPigments[] = $productEntity->toArray(BarnetEntity::$PUBLIC_LANDING);
                ...
                <div class="product__pageListing">
                    <div class="product__pageListing--row">
                        ...
                        <div class="product__pageListing--colRight">
                            <?php the_content(); ?>
                            <?php
                            $listPostPigments = array();
                            $index = 0;
                            foreach ($postsPigments as $postPigments) {
                                if ($postPigments['data']['product_featured'] != 1) {
                                    continue;
                                }
                                ...
                                $listPostPigments[] = $postPigments;
                            }
                            ?>
                            <?php if (count($listPostPigments) > 0) : ?>
                            <div class="product__title-widget --mg-top-lg">Featured Pigments</div>
                            <?php endif;?>
                            <div class="product__wrapping --mg-top-sm">
                                ...
                                <div class="product__listing" data-product-type="Pigments">
                                    <div class="component-list-product">
                                        <?php
                                        foreach ($listPostPigments as $postPigments) {
                                            ?>
                                            <div class="component-list-product__item">
                                                <div class="component-list-product__wrap">
                                                    <div class="component-list-product__title">
                                                        <h3><a href="<?php echo get_permalink($postPigments['data']['id']) ?>"
                                                            title="<?php echo $postPigments['data']['post_title']; ?>"
                                                            rel="stylesheet">
                                                                <?php echo $postPigments['data']['post_title']; ?>
                                                                <?php if (!empty($postPigments['data']['product_area']) && $postPigments['data']['product_area'] == "global") :?>
                                                                    <i class="icon icon-global-product"></i>
                                                                <?php endif;?>
                                                            </a>
                                                        </h3>
                                                    </div>
                                                    <div class="component-list-product__desc"><?php echo $postPigments['data']['post_excerpt']; ?></div>
                                                </div>
                                            </div>
                                            <?php
                                        }
